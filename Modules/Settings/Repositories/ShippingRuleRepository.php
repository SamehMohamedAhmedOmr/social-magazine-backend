...
class ShippingRuleRepository extends LaravelRepositoryClass
{
    public function __construct(ShippingRules $shipping_rule)
    {
        $this->model = $shipping_rule;
        $this->cache_key = 'shipping_rule';
    }
    ...
    private function filtering($search_keys){
        ...
        if ($search_keys) {
            $query = $query->where(function ($q) use ($search_keys){
                $q->where('shipping_rule_label', 'LIKE', '%'.$search_keys.'%')
